...
NerfDataset load_nerfslam(const std::vector<filesystem::path>& jsonpaths, float sharpen_amount) {
	...
	tlog::info() << "Loading NeRF SLAM dataset from";
	...
	for (size_t i = 0; i < jsons.size(); ++i) {
		...
		if (json.contains("envmap") && result.envmap_resolution.isZero()) {
			...
			if (equals_case_insensitive(envmap_path.extension(), "exr")) {
				result.envmap_data = load_exr_gpu(envmap_path.str(), &result.envmap_resolution.x(), &result.envmap_resolution.y());
				result.is_hdr = true;
			} else {
				result.envmap_data = load_stbi_gpu(envmap_path.str(), &result.envmap_resolution.x(), &result.envmap_resolution.y());
			}
		}
...
NerfDataset NerfDataset::add_training_image(nlohmann::json frame, uint8_t *img, uint16_t *depth, uint8_t *alpha, uint8_t *mask) {
	...
	std::string path = "/media/rpng/RPNG_FLASH_4/datasets/rgbd_dataset_freiburg3_long_office_household/rgb/1341847980.822978.png";
